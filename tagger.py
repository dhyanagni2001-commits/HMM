...
def model_training(train_data, tags):
    ...
    ###################################################
    for idx, word in enumerate(sorted(unique_words.keys())):
        word2idx[word] = idx
    ...
    for line in train_data:
        ...
        pi[tag2idx[pos_tags[0]]] += 1

        for i in range(len(words)):
            s = tag2idx[pos_tags[i]]
            o = word2idx[words[i]]
            B[s, o] += 1

            if i < len(words) - 1:
                s_next = tag2idx[pos_tags[i+1]]
                A[s, s_next] += 1
    ...
    for line in test_data:
        for word in line.words:
            if word not in model.obs_dict:
                ...
                model.B = np.hstack((model.B, np.full((len(model.pi), 1), 1e-6)))
